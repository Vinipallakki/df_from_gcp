...
schema = [
    ...
    bigquery.SchemaField("total_value", "FLOAT"),  # or "INTEGER" based on your data
]
...
with DAG(
    ...
    # Task 1: Create BigQuery table
    create_table = BigQueryCreateTableOperator(
        task_id='create_bigquery_table',
        dataset_id='first',
        table_id='processed_sales_data',
        schema_fields=schema,
        project_id='banded-edge-437103-i9',
        time_partitioning=None  # Set this if you want to partition the table
    )

    # Task 2: Submit Dataproc job
    dataproc_job = {
    ...
    submit_dataproc_job = DataprocSubmitJobOperator(
        ...
        job=dataproc_job,
        region='us-central1'  # Use 'region' instead of 'location'
    )

    # Task 3: Load processed data from GCS to BigQuery
    load_to_bq = GCSToBigQueryOperator(
        task_id='load_to_bigquery',
        bucket='banded-edge-437103-i9',
        source_objects=['processed_data/processed_output.csv'],
        destination_project_dataset_table='banded-edge-437103-i9.first.processed_sales_data',
        source_format='CSV',
        skip_leading_rows=1,
    ...
    start >> create_table >> submit_dataproc_job >> load_to_bq
