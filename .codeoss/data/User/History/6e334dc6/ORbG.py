...
def format_output(element):
    """Format the output for BigQuery."""
...
def run():
    ...
    lines = (
        ...
        | 'Window into fixed intervals' >> beam.WindowInto(FixedWindows(5))  # Fixed window of 5 seconds
        | 'Key by name' >> beam.Map(lambda x: (x['name'], x))  # Keying by name or any attribute
        | 'Group by key' >> beam.GroupByKey()  # Grouping by key
        | 'Format Output' >> beam.Map(lambda x: list(x[1]))  # Flatten the grouped data
    )
